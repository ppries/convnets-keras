...
if __name__ == "__main__":
    ...
    print 'Loading model'
    sgd = SGD(lr=0.1, decay=1e-6, momentum=0.9, nesterov=True)
    model = convnet('vgg_19', weights_path="weights/vgg19_weights.h5", heatmap=True)
    print 'Compiling'
    model.compile(optimizer=sgd, loss='mse')

    root = '/home/philip/data/recoord/frames-1080p'
    # root = '/Users/ppries/data/recoord/frames-1080p'
    for p in os.listdir(root):
        print 'Preprocessing', p
        im = preprocess_image_batch([os.path.join(root, p)], color_mode='bgr')
        print 'Predicting'
        out = model.predict(im)
        print 'Creating and saving heatmap'
        heatmap = out[0, ids, :, :].sum(axis=0) * 255
        imsave(os.path.join('heatmaps', p), heatmap)
